...
public class PlanDefinitionProcessor {
  ...
  private Goal convertGoal(PlanDefinition.PlanDefinitionGoalComponent goal) {
    ...
    var goalTarget = goal.hasTarget()
      ? goal.getTarget().stream().map((target) -> {
  ...
  private void applyQuestionnaireDefinition(PlanDefinition planDefinition, RequestGroup requestGroup, Session session, Reference definition, PlanDefinition.PlanDefinitionActionComponent action) {
  ...
  private void applyActivityDefinition(PlanDefinition planDefinition, RequestGroup requestGroup, Session session, Reference definition,
      PlanDefinition.PlanDefinitionActionComponent action) {
    IBaseResource result;
    try {
      var referenceToContained = definition.getReference().startsWith("#");
  ...
  private void applyNestedPlanDefinition(RequestGroup requestGroup, Session session, Reference definition,
      PlanDefinition.PlanDefinitionActionComponent action) {
  ...
  private void resolveDynamicActions(PlanDefinition planDefinition, RequestGroup requestGroup, Session session, PlanDefinition.PlanDefinitionActionComponent action) {
    for (PlanDefinition.PlanDefinitionActionDynamicValueComponent dynamicValue : action.getDynamicValue()) {
      ...
      if (dynamicValue.hasLanguage()) {
        ...
        var libraryToBeEvaluated = ensureLibrary(planDefinition);
        var language = dynamicValue.getLanguage();
        var result = evaluateConditionOrDynamicValue(dynamicValue.getExpression(), language, libraryToBeEvaluated, session, action.getInput());
  ...
  private Boolean meetsConditions(PlanDefinition planDefinition, RequestGroup requestGroup, Session session, PlanDefinition.PlanDefinitionActionComponent action) {
    ...
    for (var condition : action.getCondition()) {
      ...
      if (condition.hasLanguage()) {
        ...
        var libraryToBeEvaluated = ensureLibrary(planDefinition);
        var language = condition.getLanguage();
        // var bundleFactory = fhirContext.newBundleFactory();
        var result = evaluateConditionOrDynamicValue(condition.getExpression(), language, libraryToBeEvaluated, session, action.getInput());
        if (result == null) {
  ...
  protected String ensureLibrary(PlanDefinition planDefinition) {
    if (planDefinition.getLibrary().size() == 1) {
      return planDefinition.getLibrary().get(0).getReference();
    }
  ...
  protected static String getResourceName(Reference reference) {
    ...
    throw new RuntimeException("Reference must have a value for resource name extraction");
  }
