...
import org.opencds.cqf.fhir.cr.hapi.cdshooks.svc.cr.CdsCrSettings;
...
public interface ICdsConfigService {
	...
	default RequestDetails createRequestDetails(FhirContext fhirContext, String id, String resourceType) {
		SystemRequestDetails rd = new SystemRequestDetails();
		rd.setServer(getRestfulServer());
		rd.setResponse(new SystemRestfulResponse(rd));
		rd.setId(Ids.newId(fhirContext.getVersion().getVersion(), resourceType, id));
		return rd;
	}
}
