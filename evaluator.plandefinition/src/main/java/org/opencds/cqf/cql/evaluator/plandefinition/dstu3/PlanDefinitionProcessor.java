...
public class PlanDefinitionProcessor extends BasePlanDefinitionProcessor<PlanDefinition> {
    ...
    public CarePlan transformToCarePlan(IBaseResource rg) {
        ...
        var carePlan = new CarePlan().setStatus(CarePlan.CarePlanStatus.DRAFT)
                ...
                .setDefinition(requestGroup.getDefinition()).setSubject(requestGroup.getSubject());
        if (encounterId != null)
            ...
            for (var actionInput : action.getInput()) {
                if (actionInput.hasProfile()) {
                    ((Questionnaire) this.questionnaire)
                            .addItem(this.questionnaireProcessor.generateItem(actionInput,
    ...
    private void applyQuestionnaireDefinition(PlanDefinition planDefinition,
            RequestGroup requestGroup, Reference definition,
            PlanDefinition.PlanDefinitionActionComponent action) {
        ...
        try {
            ...
            if (referenceToContained) {
                ...
                var iterator =
                        fhirDal.searchByUrl("Questionnaire", definition.getReference()).iterator();
                if (!iterator.hasNext()) {
    ...
    private void applyActivityDefinition(PlanDefinition planDefinition, RequestGroup requestGroup,
            Reference definition, PlanDefinition.PlanDefinitionActionComponent action) {
        IBaseResource result;
        try {
            var referenceToContained = definition.getReference().startsWith("#");
            if (referenceToContained) {
                ...
                var iterator = fhirDal.searchByUrl("ActivityDefinition", definition.getReference())
                        .iterator();
                if (!iterator.hasNext()) {
                    throw new FHIRException(
    ...
    private void resolveTask(RequestGroup requestGroup, Task task,
        ...
        if (action.hasRelatedAction()) {
            ...
            for (var relatedAction : relatedActions) {
                ...
                var targetRef =
                        new Reference(new IdType(task.fhirType(), relatedAction.getActionId()));
    ...
    private List<Bundle> getQuestionnaireForOrder(
        ...
        var prepopulateExtension =
                getExtensionByUrl(action, Constants.SDC_QUESTIONNAIRE_PREPOPULATE);
        ...
        var urlExtension =
                getExtensionByUrl(action, Constants.SDC_QUESTIONNAIRE_LOOKUP_QUESTIONNAIRE);
        ...
        if (questionnaireUrl.contains("$")) {
            ...
            try {
                bundle = client.operation().onInstance(new IdType("Patient"))
                        .named("$" + urlSplit[1])
                        .withParameters(new Parameters().addParameter(
                                new ParametersParameterComponent(new StringType("order"))
                                        .setValue(new StringType(orderId))))
    ...
    private Boolean meetsConditions(PlanDefinition planDefinition, RequestGroup requestGroup,
        ...
        for (var condition : action.getCondition()) {
            if (condition.hasExpression()) {
                var result = resolveCondition(condition.getLanguage(), condition.getExpression(),
                        null, null, planDefinition.getLibrary().get(0).getReference(),
                        resolveInputParameters(action.getInput()));
                if (result == null) {
                    ...
                    if (req.hasCodeFilter()) {
                        for (var filter : req.getCodeFilter()) {
                            ...
                            if (filter != null && filter.hasPath() && filter.hasValueSet()) {
                                var valueset = fhirDal.searchByUrl("ValueSet",
                                        filter.getValueSet().getId());
