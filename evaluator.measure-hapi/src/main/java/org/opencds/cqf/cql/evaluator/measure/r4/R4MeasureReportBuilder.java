...
public class R4MeasureReportBuilder
  ...
  protected static final String MISSING_ID_NO_CRITERIA_REF_EXT = String.join(
      ...
      "Ensure all groups, populations, SDEs, and stratifiers", "in your Measure have ids set.",
      " ");
  ...
  protected void buildStratumPopulation(BuilderContext bc, StratifierGroupPopulationComponent sgpc,
      List<String> subjectIds, MeasureGroupPopulationComponent population) {
      ...
      if (!(accumulator.getKey().getValue() instanceof Resource)) {
        ...
        switch (report.getType()) {
          case INDIVIDUAL:
            ...
            break;
          default:
            obs = createPopulationObservation(bc, UUID.randomUUID().toString(), sde.id(),
                valueCoding, valueCount, concept);
            break;
        }
  ...
  protected Observation createObservation(BuilderContext bc, String id, String populationId) {
    var measure = bc.measure();
    MeasureInfo measureInfo = new MeasureInfo().withMeasure(measure.hasUrl() ? measure.getUrl()
