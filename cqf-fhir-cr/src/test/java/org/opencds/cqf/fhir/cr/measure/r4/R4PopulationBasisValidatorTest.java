...
class R4PopulationBasisValidatorTest {
    ...
    //    // Not ENTIRELY realistic since the GroupDefs are ultimately sourced from a MeasureDef, but for this
    // simplistic
    //    // test, it works
    ...
    //    private static final CodeDef BASIS_ENCOUNTER = new CodeDef(MeasureConstants.POPULATION_BASIS_URL,
    // "Encounter");
    //    private static final CodeDef BASIS_PROCEDURE = new CodeDef(MeasureConstants.POPULATION_BASIS_URL,
    // "Procedure");
    ...
    //                                EXPRESSION_NUMERATOR,
    //                                List.of(ENCOUNTER))),
    //                        "group expression criteria results for expression: [InitialPopulation] and scoring:
    // [PROPORTION] must fall within accepted types for population basis: [boolean] for Measure: [fakeMeasureUrl] due to
    // mismatch between total result classes: [Encounter] and matching result classes: []"),
    //                new ValidateGroupBasisTypeErrorPathParams(
    //                        buildGroupDef(
    //                                Basis.BOOLEAN,
    //                                buildPopulationDefs(Basis.BOOLEAN, INITIALPOPULATION, DENOMINATOR, NUMERATOR),
    ...
    //                        "group expression criteria results for expression: [InitialPopulation] and scoring:
    // [PROPORTION] must fall within accepted types for population basis: [boolean] for Measure: [fakeMeasureUrl] due to
    // mismatch between total result classes: [Boolean, Boolean, Encounter] and matching result classes: [Boolean,
    // Boolean]"),
    //                new ValidateGroupBasisTypeErrorPathParams(
    //                        buildGroupDef(
    //                                Basis.BOOLEAN,
    ...
    //                                EXPRESSION_NUMERATOR,
    //                                List.of(Boolean.TRUE, Boolean.TRUE, ENCOUNTER))),
    //                        "group expression criteria results for expression: [Numerator] and scoring: [PROPORTION]
    // must fall within accepted types for population basis: [boolean] for Measure: [fakeMeasureUrl] due to mismatch
    // between total result classes: [Boolean, Boolean, Encounter] and matching result classes: [Boolean, Boolean]"),
    //                new ValidateGroupBasisTypeErrorPathParams(
    //                        buildGroupDef(
    //                                Basis.ENCOUNTER,
    //                                buildPopulationDefs(Basis.ENCOUNTER, INITIALPOPULATION, DENOMINATOR, NUMERATOR),
    ...
    //                                Boolean.TRUE)),
    //                        "group expression criteria results for expression: [InitialPopulation] and scoring:
    // [PROPORTION] must fall within accepted types for population basis: [Encounter] for Measure: [fakeMeasureUrl] due
    // to mismatch between total result classes: [Boolean] and matching result classes: []"),
    //                new ValidateGroupBasisTypeErrorPathParams(
    //                        buildGroupDef(
    //                                Basis.ENCOUNTER,
    //                                buildPopulationDefs(Basis.ENCOUNTER, INITIALPOPULATION, DENOMINATOR, NUMERATOR),
    ...
    //                                List.of(ENCOUNTER))),
    //                        "group expression criteria results for expression: [InitialPopulation] and scoring:
    // [PROPORTION] must fall within accepted types for population basis: [Procedure] for Measure: [fakeMeasureUrl] due
    // to mismatch between total result classes: [Encounter] and matching result classes: []"),
    //                new ValidateGroupBasisTypeErrorPathParams(
    //                        buildGroupDef(
    //                                Basis.ENCOUNTER,
    //                                buildPopulationDefs(Basis.ENCOUNTER, INITIALPOPULATION, DENOMINATOR, NUMERATOR),
    ...
    //                        "group expression criteria results for expression: [Numerator] and scoring: [PROPORTION]
    // must fall within accepted types for population basis: [Encounter] for Measure: [fakeMeasureUrl] due to mismatch
    // between total result classes: [Encounter, Procedure, Encounter] and matching result classes: [Encounter,
    // Encounter]"));
    //    }
    //
    //    @ParameterizedTest(name = "{index} => testCase={0}")
    ...
    //     */
    //    private record ValidateStratifierBasisTypeHappyPathParams(GroupDef groupDef, EvaluationResult
    // evaluationResult) {}
    //
    ...
    //                "stratifier expression criteria results for expression: [InitialPopulation] must fall within
    // accepted types for population-basis: [boolean] for Measure: [fakeMeasureUrl] due to mismatch between total eval
    // result classes: [Encounter] and matching result classes: []";
    //
    //        validateStratifierBasisTypeErrorPath(expectedGroupDef, expectedEvaluationResult,
    ...
    //                "stratifier expression criteria results for expression: [InitialPopulation] must fall within
    // accepted types for population-basis: [boolean] for Measure: [fakeMeasureUrl] due to mismatch between total eval
    // result classes: [Encounter, Encounter, Encounter] and matching result classes: []";
    //
    //        validateStratifierBasisTypeErrorPath(expectedGroupDef, expectedEvaluationResult,
    // expectedExceptionMessage);
    //    }
    ...
    //
    //        var expectedExceptionMessage =
    //                "stratifier expression criteria results for expression: [Numerator] must fall within accepted
    // types for population-basis: [boolean] for Measure: [fakeMeasureUrl] due to mismatch between total eval result
    // classes: [Boolean, Boolean, Encounter] and matching result classes: [Boolean, Boolean]";
    //
    //        validateStratifierBasisTypeErrorPath(expectedGroupDef, expectedEvaluationResult,
    // expectedExceptionMessage);
    //    }
    ...
    //        return new GroupDef(
    //                null, null, stratifierDefs, populationDefs, MeasureScoring.PROPORTION, false, null,
    // basis.codeDef);
    //    }
    //
    //    @Nonnull
    ...
    //
    //    @Nonnull
    //    private static EvaluationResult buildEvaluationResult(Map<String, Value> expressionResultMap) {
