...
public class DefaultDataProviderFactory implements DataProviderFactory {
    ...
    public Map<String, DataProvider> create(Map<VersionedIdentifier, Library> libraries, Map<String,String> modelUris, TerminologyProvider terminologyProvider) {
    ...
    private  Map<String, Pair<String, String>> getVersions(Map<VersionedIdentifier, Library> libraries, Map<String,String> modelUris) {
        Map<String, Pair<String, String>> versions = new HashMap<>();
        for (Map.Entry<String, String> modelUri : modelUris.entrySet()) {
            String uri = shorthandMap.containsKey(modelUri.getKey()) ? 
                shorthandMap.get(modelUri.getKey()) :
                modelUri.getKey();
            ...
            if (version == null) {
                throw new IllegalArgumentException(String.format("A uri was specified for %s but is not used.", modelUri.getKey()));
            }

            if (versions.containsKey(uri)) {
                if (!versions.get(uri).getKey().equals(version)) {
                    throw new IllegalArgumentException(String.format("Libraries are using multiple versions of %s. Only one version is supported at a time.", modelUri.getKey()));
                }
    ...
    private Map<String, DataProvider> getProviders(Map<String, Pair<String, String>> versions, TerminologyProvider terminologyProvider) {
        Map<String, DataProvider> providers = new HashMap<>();
        for (Map.Entry<String, Pair<String, String>> m : versions.entrySet()){
            providers.put(m.getKey(), this.getProvider(m.getKey(), m.getValue().getLeft(), m.getValue().getRight(), terminologyProvider));
        }

        return providers;
    }

    private DataProvider getProvider(String model, String version, String uri, TerminologyProvider terminologyProvider) {
        switch(model) {
        ...
        switch (version) {
            case "3.0.0":
                if (Helpers.isFileUri(uri)) {
                    ...
                    FhirDataProviderStu3 provider = new FhirDataProviderStu3();
                    provider.setEndpoint(uri);
