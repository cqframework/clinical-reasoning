 ...
 * <a href="https://www.hl7.org/fhir/http.html">FHIR HTTP API</a>, but constrained to provide only
 * the operations necessary for the cql-evaluator modules to function.
 *
 * @see <a href="https://www.hl7.org/fhir/http.html">https://www.hl7.org/fhir/http.html</a>
 */
public interface FhirDal {
  /**
   * Fetches an {@code IBaseResource} by {@code id}. The {@code IIdType} must have the resourceType
   * defined.
   *
  ...
  /**
   * Deletes an {@code IBaseResource} by {@code id}. The {@code IIdType} must have the resourceType
   * defined.
   *
  ...
  /**
   * Returns an {@code Iterable<IBaseResource>} of all the resources of type {@code resourceType}
   * with a url property value of {@code url}.
   *
   * Only knowledge artifact resources have the url property. A search on a non-knowledge artifact
   * resource will throw an {@code IllegalArgumentException}
   *
   * Default behavior is to return null if no resources are found.
   *
   * @param resourceType the type of resources to return.
