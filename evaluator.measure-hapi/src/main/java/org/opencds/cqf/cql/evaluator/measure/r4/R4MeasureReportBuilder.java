...
public class R4MeasureReportBuilder implements MeasureReportBuilder<Measure, MeasureReport, DomainResource> {
    ...
    protected void buildStratum(StratifierGroupComponent stratum, Object value, Set<String> subjectIds,
        ...
        stratum.setValue(new CodeableConcept().setText(value.toString()));
    ...
    protected void buildStratumPopulation(StratifierGroupPopulationComponent sgpc, Set<String> subjectIds, MeasureGroupPopulationComponent population) {
        ...
        ListResource popSubjectList = this.createSubjectIdList(intersection);
    ...
    protected ListResource createSubjectList(Set<Object> subjects) {
        return this.createSubjectIdList(
                subjects.stream().map(x -> ((DomainResource) x).getId()).collect(Collectors.toList()));
    }

    protected ListResource createSubjectIdList(Collection<String> subjectIds) {
        ListResource subjectList = createList();
        for (String id : subjectIds) {
            subjectList.addEntry().setItem(new Reference().setReference(id));
        }
