...
public class R4PackageService {
     ...
     *                                      resulting bundle will be of type searchset.
    ...
    public Bundle packageOperation(
        ...
        retVal.getEntry().stream()
                ...
                .filter(r -> {
                    ...
                    var id2 = r.getResourceType().toString() + "/" + id.getIdPart();
                    return id1.equals(id2);
                })
                .findFirst()
                .ifPresent(m -> KnowledgeArtifactProcessor.handleValueSetReferenceExtensions(m, retVal.getEntry()));
        //        // we don't forward these `count` or `offset` parameters to `clinical-reasoning` so that
        //        // we can handle VSM-specific operations here using the entire packaged bundle
