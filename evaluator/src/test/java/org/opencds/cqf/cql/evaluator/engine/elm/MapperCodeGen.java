...
public class MapperCodeGen {
  ...
  public static void main(String[] args) {
    ...
    Set<Class<? extends org.cqframework.cql.elm.execution.Element>> engineSubTypes =
        engineTypeReflections.getSubTypesOf(org.cqframework.cql.elm.execution.Element.class);
    ...
    for (Map.Entry<Class<? extends Element>, Class<? extends org.cqframework.cql.elm.execution.Element>> entry : typeMap
      ...
      System.out
          .println(entry.getValue().getName() + " map(" + entry.getKey().getName() + " element);");
    }
  ...
  static void generateFunctionForAbstractType(Class<? extends Element> abstractType,
    ...
    String abstractEngineName =
        abstractType.getName().replace("org.hl7.elm.r1", "org.cqframework.cql.elm.execution");
    System.out.println(
    ...
    concreteSubClasses.sort((x, y) -> Long
        .valueOf(concreteSubClasses.stream().filter(z -> x.isAssignableFrom(z)).count()).compareTo(
            Long.valueOf(concreteSubClasses.stream().filter(z -> y.isAssignableFrom(z)).count())));
    Boolean first = true;
