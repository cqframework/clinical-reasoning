...
public abstract class BasePlanDefinitionProcessor<T> {
  ...
  public IBase getExpressionResult(String expression, String language, String libraryToBeEvaluated,
    ...
    switch (language) {
      ...
      case "text/cql-name":
        parametersResult =
            libraryProcessor.evaluate(libraryToBeEvaluated, patientId, parameters, contentEndpoint,
                terminologyEndpoint, dataEndpoint, bundle, Collections.singleton(expression));
        result = (IBase) resolveParameterValue(ParametersUtil
            .getNamedParameter(fhirContext, parametersResult, expression).orElse(null));
        break;
      case "text/fhirpath":
