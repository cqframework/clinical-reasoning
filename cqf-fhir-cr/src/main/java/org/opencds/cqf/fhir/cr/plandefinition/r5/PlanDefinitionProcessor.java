    ...
    private RequestOrchestrationActionComponent resolveAction(
        ...
        if (planDefinition.hasExtension(Constants.CPG_QUESTIONNAIRE_GENERATE) && action.hasInput()) {
            for (var actionInput : action.getInput()) {
                if (actionInput.getRequirement().hasProfile()) {
                    addQuestionnaireItemForInput(actionInput.getRequirement());
                }
            }
        }

        if (Boolean.TRUE.equals(meetsConditions(defaultLibraryUrl, action, requestOrchestration))) {
            ...
            extensionResolver.resolveExtensions(requestOrchestration, requestAction.getExtension(), defaultLibraryUrl);
            if (action.hasAction()) {
            ...
            resolveDynamicValues(defaultLibraryUrl, requestAction, resource, action, requestOrchestration);
