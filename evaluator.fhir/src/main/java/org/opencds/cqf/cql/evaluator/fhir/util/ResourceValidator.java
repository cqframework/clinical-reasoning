...
public class ResourceValidator {
  ...
  protected void setValidator() {
    if (this.profiles.isEmpty()) {
      ...
      supportChain
          .addValidationSupport(new InMemoryTerminologyServerValidationSupport(this.context));
  ...
  public IBaseResource validate(IBaseResource resource, Boolean error) {
    ...
    var errors =
        validationResult.getMessages().stream()
            .filter(m -> m.getSeverity().compareTo(ResultSeverityEnum.ERROR) > -1 && this.profiles
