...
public class ExpressionProcessor {
    ...
    CqfExpression getInitialExpression(Questionnaire questionnaire, QuestionnaireItemComponent item) {
        if (item.hasExtension(Constants.CQF_EXPRESSION)) {
            ...
            return prePopulateHelper.getExpressionByExtension(
                    questionnaire, item, Constants.SDC_QUESTIONNAIRE_INITIAL_EXPRESSION);
        }
        return null;
    }

    List<IBase> getExpressionResult(PrePopulateRequest prePopulateRequest, CqfExpression expression, String itemLinkId)
