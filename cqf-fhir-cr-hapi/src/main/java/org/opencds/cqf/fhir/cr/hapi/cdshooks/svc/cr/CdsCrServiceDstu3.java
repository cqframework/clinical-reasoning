...
public class CdsCrServiceDstu3 implements ICdsCrService {
	...
	public CdsServiceResponseJson encodeResponse(Object response) {
		...
		this.response = (CarePlan) response;
		CdsServiceResponseJson serviceResponse = new CdsServiceResponseJson();
		if (this.response.hasActivity()) {
			Reference requestGroupRef = this.response.getActivity().get(0).getReference();
	...
	protected void resolveSystemAction(RequestGroup.RequestGroupActionComponent action) {
		if (action.hasType()
			...
			serviceResponse.addServiceAction(new CdsServiceResponseSystemActionJson()
					...
					.setType(action.getType().getCode()));
		}
	}
	...
	protected IBaseResource resolveResource(Reference reference) {
		return response.getContained().stream()
				.filter(resource -> resource.getId().equals(reference.getReference()))
				.findFirst()
				.orElse(null);
	}
}
