...
public class KnowledgeArtifactReleaseVisitor implements r4KnowledgeArtifactVisitor {
    ...
    public IBase visit(r4LibraryAdapter rootLibraryAdapter, Repository repository, Parameters operationParameters) {
        ...
        String releaseLabel = MetadataResourceHelper.getParameter("releaseLabel", operationParameters, StringType.class)
                .map(t -> t.getValue())
                .orElse("");
        Optional<CodeType> versionBehavior =
                MetadataResourceHelper.getParameter("versionBehavior", operationParameters, CodeType.class);
        Optional<CodeType> requireNonExpermimental =
                MetadataResourceHelper.getParameter("requireNonExperimental", operationParameters, CodeType.class);
        CRMIReleaseVersionBehaviorCodes versionBehaviorCode;
        CRMIReleaseExperimentalBehaviorCodes experimentalBehaviorCode;
        try {
            versionBehaviorCode = versionBehavior.isPresent()
                    ? CRMIReleaseVersionBehaviorCodes.fromCode(
                            versionBehavior.get().getCode())
                    : CRMIReleaseVersionBehaviorCodes.NULL;
            experimentalBehaviorCode = requireNonExpermimental.isPresent()
                    ? CRMIReleaseExperimentalBehaviorCodes.fromCode(
                            requireNonExpermimental.get().getCode())
        ...
        String existingVersion =
                rootLibrary.hasVersion() ? rootLibrary.getVersion().replace("-draft", "") : null;
        ...
        List<MetadataResource> releasedResources = internalRelease(
                rootLibraryAdapter,
                releaseVersion,
                rootEffectivePeriod,
                versionBehaviorCode,
        ...
        rootLibraryAdapter
                .getRelatedArtifact()
                .removeIf(ra -> ((RelatedArtifact) ra).getType() == RelatedArtifact.RelatedArtifactType.DEPENDSON);
        ...
        for (MetadataResource artifact : releasedResources) {
            ...
            for (RelatedArtifact component : components) {
                ...
                if (r4KnowledgeArtifactAdapter.checkIfRelatedArtifactIsOwned(component)) {
                ...
                } else if (Canonicals.getVersion(component.getResourceElement()) == null
                        || Canonicals.getVersion(component.getResourceElement()).isEmpty()) {
                    ...
                    String updatedReference = tryUpdateReferenceToLatestActiveVersion(
                            component.getResource(), repository, artifact.getUrl());
                    component.setResource(updatedReference);
                }
                RelatedArtifact componentToDependency = new RelatedArtifact()
                        .setType(RelatedArtifact.RelatedArtifactType.DEPENDSON)
                        .setResource(component.getResourceElement().getValueAsString());
            ...
            for (DependencyInfo dependency : dependencies) {
                ...
                if (maybeReference.isPresent()) {
                    String updatedReference = String.format(
                            "%s|%s",
                            maybeReference.get().getUrl(), maybeReference.get().getVersion());
                    ...
                    if (Canonicals.getVersion(dependency.getReference()) == null
                        ...
                        String updatedReference = tryUpdateReferenceToLatestActiveVersion(
                                dependency.getReference(), repository, artifact.getUrl());
                        dependency.setReference(updatedReference);
                    }
                }
                ...
                if (!artifact.getUrl().equals(rootLibrary.getUrl())) {
                    ...
                    newDep.setType(RelatedArtifactType.DEPENDSON).setResource(dependency.getReference());
        ...
        for (RelatedArtifact resolvedRelatedArtifact : rootLibraryAdapter.getRelatedArtifact()) {
            ...
            if (isDistinct) {
                ...
                originalDependenciesWithExtensions.stream()
                        ...
                        .findFirst()
                        .ifPresent(dep -> {
                            resolvedRelatedArtifact
                                    .getExtension()
                                    .addAll(dep.getExtension().stream()
                                            .map(e -> (Extension) e)
                                            .collect(Collectors.toList()));
                            originalDependenciesWithExtensions.removeIf(
                                    ra -> ra.getReference().equals(resolvedRelatedArtifact.getResource()));
                        });
            }
        }
    ...
    private void checkNonExperimental(
            MetadataResource resource, CRMIReleaseExperimentalBehaviorCodes experimentalBehavior, Repository repository)
        ...
        if (resource.getResourceType().equals(ResourceType.ValueSet)) {
            ...
            List<CanonicalType> valueSets = valueSet.getCompose().getInclude().stream()
    ...
    private Optional<String> getReleaseVersion(
            String version, CRMIReleaseVersionBehaviorCodes versionBehavior, String existingVersion)
            throws UnprocessableEntityException {
    ...
    private Optional<MetadataResource> checkIfReferenceInList(
            RelatedArtifact artifactToUpdate, List<MetadataResource> resourceList) {
    ...
    private void checkReleasePreconditions(MetadataResource artifact, Date approvalDate)
        ...
        if (Enumerations.PublicationStatus.DRAFT != artifact.getStatus()) {
            throw new PreconditionFailedException(String.format(
                    ...
                    artifact.getIdElement().getIdPart()));
        }
    ...
    private List<BundleEntryComponent> findArtifactCommentsToUpdate(
        ...
        this.searchArtifactAssessmentForArtifact(rootArtifact.getIdElement(), repository).getEntry().stream()
                ...
                .forEach(artifactComment -> {
                    artifactComment.setDerivedFromContentRelatedArtifact(
                            new CanonicalType(String.format("%s|%s", rootArtifact.getUrl(), releaseVersion)));
                    returnEntries.add(PackageHelper.createEntry(artifactComment, true));
                });
        return returnEntries;
    }

    private void checkReleaseVersion(String version, CRMIReleaseVersionBehaviorCodes versionBehavior)
            throws UnprocessableEntityException {
    ...
    private String tryUpdateReferenceToLatestActiveVersion(
        ...
        List<MetadataResource> matchingResources = MetadataResourceHelper.getMetadataResourcesFromBundle(
                        (Bundle) SearchHelper.searchRepositoryByCanonicalWithPaging(
                                repository, new CanonicalType(inputReference)))
                .stream()
        ...
        if (matchingResources.isEmpty()) {
            ...
            matchingResources.sort(
                    comparing(r -> ((MetadataResource) r).getVersion()).reversed());
            MetadataResource latestActiveVersion = matchingResources.get(0);
            String latestActiveReference =
