...
### Extension propagation during definition application

`CrSettings` controls which extensions are automatically copied during `PlanDefinition/$apply`,
`PlanDefinition/$r5.apply`, and `ActivityDefinition/$apply`. The default policy, `legacy()`, preserves
the existing extension selection. Configure the policy before first using the processors:
...
var settings = CrSettings.getDefault()
        .withExtensionPropagationPolicy(ExtensionPropagationPolicy.excludeUrls(Set.of(
                "urn:example:owned-by",
                "urn:example:plan-definition/plan-default")));

var planProcessor = new PlanDefinitionProcessor(repository, settings);
var activityProcessor = new ActivityDefinitionProcessor(repository, settings);
```

The available policies are:

| Policy | Automatic copies |
|---|---|
| `legacy()` | All extensions except the processor's built-in exclusions |
| `excludeUrls(urls)` | All extensions except the built-in exclusions and the supplied exact URLs |
| `allowOnlyUrls(urls)` | Only the supplied exact URLs, still subject to built-in exclusions |
| `none()` | No extensions at the propagation sites listed below |

URL sets are defensively copied. An empty allowlist behaves like `none()`; an empty exclusion set
behaves like `legacy()`. The policy applies to PlanDefinition root extensions, action extensions
(including child actions), ActivityDefinition root extensions, and PlanDefinition goal target
extensions. Nested plans and activities inherit the policy when using the default processors.
If supplying custom operation processors, configure their propagation behavior explicitly; the
framework does not overwrite custom processor configuration.

Source extensions remain on their definitions. Accepted extensions are deep-copied before any CQF
expression resolution. A complex extension is selected as a whole, including its nested extensions.
This policy does not recursively sanitize extensions within other copied data types, filter
`modifierExtension`, or remove extensions explicitly generated by operation logic or dynamic values.

Custom policies can inspect `extensionUrl()`, `fhirVersion()`, `sourcePath()`, and `targetPath()`:

```java
settings.withExtensionPropagationPolicy(copy ->
        copy.extensionUrl().equals("urn:example:action-note")
                && copy.sourcePath().equals("PlanDefinition.action")
                && copy.targetPath().equals("RequestGroup.action"));
```

Paths describe element definitions without instance indexes. Child actions use the same
`PlanDefinition.action` and `RequestGroup.action` (or R5 `RequestOrchestration.action`) paths as their
parents. The FHIR version is the resource version, so `$r5.apply` on R4 resources still supplies R4
and `RequestGroup` paths. Shared custom policies must be thread-safe.

Propagation policies do not resolve extension StructureDefinitions or validate their declared
contexts. Select URLs and destinations appropriate to your implementation guide and continue
validating generated resources as needed.
