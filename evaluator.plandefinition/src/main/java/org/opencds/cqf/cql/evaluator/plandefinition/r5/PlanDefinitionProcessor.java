...
public class PlanDefinitionProcessor {
  ...
  private Resource resolveTask(RequestGroup requestGroup, Task task, PlanDefinition.PlanDefinitionActionComponent action) {
    ...
    if (action.hasInput()) {
      var dataRequirements = action.getInput().stream().map(PlanDefinition.PlanDefinitionActionInputComponent::getRequirement).collect(Collectors.toList());
  ...
  private void resolveDynamicActions(PlanDefinition planDefinition, RequestGroup requestGroup, Session session, PlanDefinition.PlanDefinitionActionComponent action) {
    for (PlanDefinition.PlanDefinitionActionDynamicValueComponent dynamicValue : action.getDynamicValue()) {
      ...
      if (dynamicValue.getExpression().hasLanguage()) {
        ...
        var requirements = action.getInput().stream().map(PlanDefinition.PlanDefinitionActionInputComponent::getRequirement).collect(Collectors.toList());
