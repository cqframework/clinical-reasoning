...
// LUKETODO:  how do we do this wihout bootstrapping from hapi-fhir?
 ...
 * Reasoning Module</a>. This implementation aims to be compatible with the CQF IG.
 * <p/>
 * myeId             the id of the Measure to evaluate myPeriodStart     The start of the reporting
 * period myPeriodEnd       The end of the reporting period myReportType      The type of
 * MeasureReport to generate mySubject         the subject to use for the evaluation myPractitioner
 *   the practitioner to use for the evaluation myLastReceivedOn  the date the results of this
 * measure were last received. myProductLine     the productLine (e.g. Medicare, Medicaid, etc) to
 * use for the evaluation. This is a non-standard parameter. myAdditionalData  the data bundle
...
//        @Header annotation
        // LUKETODO:  bind all 3 types... mark them as optional/nullable:   then mash it into an Either
        @IdParam IdType theId,
        // LUKETODO:  this is tricky:  how to update @OperationParam for the 2 new fields?
        // do we need a wave of a hapi-fhir update?   do we simply add these fields in the Feb release?
        // how do we justify this?
        // LUKETODO: can we some do the ZonedDateTime conversion in the constructor????
        @OperationParam(
            name = "periodStart",
    ...
    public boolean equals(Object theO) {
        ...
        return Objects.equals(myMeasure, that.myMeasure) && Objects.equals(myPeriodStart,
            that.myPeriodStart) && Objects.equals(myPeriodEnd, that.myPeriodEnd) && Objects.equals(
            myReportType, that.myReportType) && Objects.equals(mySubject, that.mySubject)
            && Objects.equals(myPractitioner, that.myPractitioner) && Objects.equals(
            myLastReceivedOn, that.myLastReceivedOn) && Objects.equals(myProductLine,
            that.myProductLine) && Objects.equals(myAdditionalData, that.myAdditionalData)
            && Objects.equals(myTerminologyEndpoint, that.myTerminologyEndpoint) && Objects.equals(
            myParameters, that.myParameters);
    }

    @Override
    public int hashCode() {
        return Objects.hash(myMeasure, myPeriodStart, myPeriodEnd, myReportType, mySubject,
            myPractitioner, myLastReceivedOn, myProductLine, myAdditionalData,
            myTerminologyEndpoint, myParameters);
    }
