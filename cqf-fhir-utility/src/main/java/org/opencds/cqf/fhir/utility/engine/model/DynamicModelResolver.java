...
public class DynamicModelResolver extends CachingModelResolverDecorator {
  ...
  public void setValue(Object target, String path, Object value) {
    if (path.contains(("."))) {
      setNestedValue((IBase) target, path, value);
    } else {
      ...
      if (targetValues.size() >= index + 1 && !isLast) {
        ...
        if (isLast) {
          ...
          if (elementClass.getSimpleName().equals(ENUMERATION)) {
            var enumFactory =
                toEnumFactory(((RuntimeChildPrimitiveEnumerationDatatypeDefinition) targetDef)
                    .getBoundEnumType());
            targetValue = (IBase) this.as(value, IPrimitiveType.class, false);
            var enumValue = enumFactory
                .fromCode(((IPrimitiveType<?>) targetValue).getValueAsString());
            super.setValue(target, path, enumValue);
            targetValue = null;
          } else {
