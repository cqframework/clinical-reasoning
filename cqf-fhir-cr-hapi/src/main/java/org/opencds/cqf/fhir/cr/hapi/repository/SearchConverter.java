...
public class SearchConverter {
    ...
    public void convertParameters(Multimap<String, List<IQueryParameterType>> parameters) {
        if (parameters == null) {
            return;
        }
        separateParameterTypes(parameters);
        convertToSearchParameterMap(separatedSearchParameters);
        convertToStringMap(separatedResultParameters);
    }

    public void convertToStringMap(@Nonnull Multimap<String, List<IQueryParameterType>> parameters) {
        for (var entry : parameters.entries()) {
            var values = new String[entry.getValue().size()];
            for (int i = 0; i < entry.getValue().size(); i++) {
                values[i] = entry.getValue().get(i).getValueAsQueryToken();
            }
