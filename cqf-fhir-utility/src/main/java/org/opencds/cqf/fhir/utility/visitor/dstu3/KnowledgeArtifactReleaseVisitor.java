...
public class KnowledgeArtifactReleaseVisitor implements KnowledgeArtifactVisitor {
    @Override
    public IBase visit(
            IBaseKnowledgeArtifactAdapter library, Repository repository, IBaseParameters operationParameters) {
    ...
    public IBase visit(IBaseLibraryAdapter library, Repository repository, IBaseParameters operationParameters) {
        ...
        // $expand valuesets (so that we can recursively pin versions for any resources referenced (e.g., CodySystem
        // references from ValueSets that we don't own)
        // The expansion will not be persisted, it is only done as a step to pin all reference versions and record them
        // in the manifest and root artifact (as 'depends-on' relatedArtifacts)
