...
public class ExpressionEvaluator {
     ...
     *        will be made available by name to the CQL expression. Parameter types are mapped to
     *        CQL as specified in the Using CQL section of the cpg implementation guide. If a
     *        parameter appears more than once in the input Parameters resource, it is represented
     *        with a List in the input CQL. If a parameter has parts, it is represented as a Tuple
     *        in the input CQL.
     * @return IBaseParameters The result of evaluating the given expression, returned as a FHIR
     *         type, either a resource, or a FHIR-defined type corresponding to the CQL return type,
     *         as defined in the Using CQL section of the cpg implementation guide. If the result is
     *         a List of resources, the result will be a Bundle. If the result is a CQL
     *         system-defined or FHIR-defined type, the result is returned as a Parameters resource
     */
    public IBaseParameters evaluate(String expression, IBaseParameters parameters) {
        return this.evaluate(expression, parameters, null, null, null, null, null, null, null,
                null);
    }
     ...
     *        context in which the expression will be evaluated and is represented as a relative
     *        FHIR id (e.g. Patient/123), which establishes both the context and context value for
     *        the evaluation
     ...
     *        parameter is true (the default), then the operation will use data first from any
     *        bundles provided as parameters (through the data and prefetch parameters), second data
     *        from the server performing the operation, and third, data from the dataEndpoint
     *        parameter (if provided). If this parameter is false, the operation will use data first
     *        from the bundles provided in the data or prefetch parameters, and second from the
     *        dataEndpoint parameter (if provided).
     * @param bundle Data to be made available to the library evaluation. This parameter is
     *        exclusive with the prefetchData parameter (i.e. either provide all data as a single
     *        bundle, or provide data using multiple bundles with prefetch descriptions).
     * @param prefetchData Data to be made available to the library evaluation, organized as
     *        prefetch response bundles. Each prefetchData parameter specifies either the name of
     *        the prefetchKey it is satisfying, a DataRequirement describing the prefetch, or both.
     * @param dataEndpoint An endpoint to use to access data referenced by retrieve operations in
     *        the library. If provided, this endpoint is used after the data or prefetchData
     *        bundles, and the server, if the useServerData parameter is true.
     * @param contentEndpoint An endpoint to use to access content (i.e. libraries) referenced by
     *        the library. If no content endpoint is supplied, the evaluation will attempt to
    ...
    public IBaseParameters evaluate(String expression, IBaseParameters parameters, String subject,
            ...
            List<Triple<String, DataRequirement, IBaseBundle>> prefetchData,
        ...
        return libraryProcessor.evaluate(
                new VersionedIdentifier().withId("expression").withVersion("1.0.0"), subject,
