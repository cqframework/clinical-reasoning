...
public class R5FhirModelResolver extends FhirModelResolver<Base, BaseDateTimeType, TimeType, SimpleQuantity, IdType, Resource, Enumeration<?>, EnumFactory<?>> {
   ...
   protected void initialize() {
      ...
      this.fhirContext.getResourceDefinition(Enumerations.ResourceTypeEnum.ACCOUNT.toCode());
      ...
      try {
         ...
         for (Enumerations.ResourceTypeEnum type : Enumerations.ResourceTypeEnum.values()) {
   ...
   public Object as(Object value, Class<?> type, boolean isStrict) {
      ...
      if (value instanceof UriType) {
         ...
         switch (type.getSimpleName()) {
            case "UrlType": return uriType.hasPrimitiveValue() ? new UrlType(uriType.primitiveValue()) : null;
            case "CanonicalType": return uriType.hasPrimitiveValue() ? new CanonicalType(uriType.primitiveValue()) : null;
            case "AnnotatedUuidType":
      ...
      if (value instanceof StringType) {
         ...
         switch (type.getSimpleName()) {
            case "CodeType": return stringType.hasPrimitiveValue() ? new CodeType(stringType.primitiveValue()) : null;
            case "MarkdownType": return stringType.hasPrimitiveValue() ? new MarkdownType(stringType.primitiveValue()) : null;
      ...
      if (value instanceof Quantity) {
         ...
         switch (type.getSimpleName()) {
            ...
            case "Count":
               ...
               return count;
            case "SimpleQuantity": return castToSimpleQuantity(quantity); // NOTE: This is wrong in that it is copying the comparator, it should be ensuring comparator is not set...
            case "MoneyQuantity":
