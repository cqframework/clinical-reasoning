...
public class QuestionnaireProcessor extends BaseQuestionnaireProcessor<Questionnaire> {
    ...
    private CqfExpression getInitialExpression(QuestionnaireItemComponent item) {
        ...
        return new CqfExpression(
                ...
                expressionExtension.getValue().toString(),
                libraryUrl);
    }
    ...
    private CqfExpression getContextExpression(QuestionnaireItemComponent item) {
        var contextExpressionExt = item.getExtensionByUrl(Constants.SDC_QUESTIONNAIRE_ITEM_POPULATION_CONTEXT);
        ...
        return new CqfExpression(
                languageExtension.getValue().toString(),
                contextExpressionExt.getValue().toString(),
                libraryUrl);
    }
    ...
    protected List<QuestionnaireItemComponent> processItemWithContext(QuestionnaireItemComponent groupItem) {
        ...
        var populationContext = getExpressionResult(getContextExpression(groupItem), groupItem.getLinkId());
            ...
            for (var item : contextItem.getItem()) {
                ...
                if (initialProperty.hasValues()) {
                    if (initialProperty.isList()) {
                        ...
                        item.addExtension(
                                Constants.QUESTIONNAIRE_RESPONSE_AUTHOR, new Reference(Constants.CQL_ENGINE_DEVICE));
                        item.setInitial(transformValue(
                                (Type) initialProperty.getValues().get(0)));
                    }
                }
            }
