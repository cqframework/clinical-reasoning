...
public class R4MeasureReportBuilderTest {
    ...
    public void checkIfNotBooleanBasedMeasure() {
        ...
        Measure measureEncounterBias = (Measure) parser.parseResource(R4MeasureReportBuilderTest.class
                .getResourceAsStream("MeasureBuilderSampleWithPopulationBiasEncounter.json"));
        ...
        assertTrue(this.measureReportBuilder.ifIsBooleanBasis(measureEncounterBias));

        assertNotNull(measureWithoutExtension);
        assertFalse(this.measureReportBuilder.ifIsBooleanBasis(measureWithoutExtension));

        assertNotNull(measureBooleanBias);
        assertFalse(this.measureReportBuilder.ifIsBooleanBasis(measureBooleanBias));

        assertNotNull(measureWithEmptyExtension);
        assertFalse(this.measureReportBuilder.ifIsBooleanBasis(measureWithEmptyExtension));
