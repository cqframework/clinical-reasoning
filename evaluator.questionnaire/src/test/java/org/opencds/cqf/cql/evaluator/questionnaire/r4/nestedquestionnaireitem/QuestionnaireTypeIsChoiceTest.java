...
class QuestionnaireTypeIsChoiceTest {
  ...
  void addPropertiesShouldAddPropertiesIfValueSetHasExpansion() throws QuestionnaireParsingException {
    ...
    doReturn(valueSet).when(parsingService).getValueSet(VALUE_SET_URL);
    // execute
    ...
    verify(parsingService).getValueSet(VALUE_SET_URL);
  ...
  void assertQuestionnaireItemsGetAppendedFromExpansionComponent(ValueSet valueSet, QuestionnaireItemComponent questionnaireItem) {
    final List<ValueSetExpansionContainsComponent> expansion = valueSet.getExpansion().getContains();
    Assert.assertEquals(expansion.size(), questionnaireItem.getAnswerOption().size());
    for (int i = 0; i < expansion.size(); i++) {
      final ValueSetExpansionContainsComponent containsComponent = expansion.get(i);
      final QuestionnaireItemAnswerOptionComponent answerOptionComponent = questionnaireItem.getAnswerOption().get(i);
  ...
  void assertQuestionnaireItemsGetAppendedFromConceptSets(ValueSet valueSet, QuestionnaireItemComponent questionnaireItem) {
    ...
    for (ConceptSetComponent theConceptSet : conceptSets) {
      ...
      for (ConceptReferenceComponent theConceptReferenceComponent : conceptReferenceComponents) {
        totalItems++;
        final QuestionnaireItemAnswerOptionComponent answerOptionComponent = questionnaireItem
            .getAnswerOption()
            .get(totalItems - 1);
        final Coding coding = (Coding) answerOptionComponent.getValue();
    ...
    Assert.assertEquals(totalItems, questionnaireItem.getAnswerOption().size());
  }
  ...
  ElementDefinition withElementDefinitionWithBindingComponent() {
    ...
    final ElementDefinitionBindingComponent elementDefinitionBindingComponent = new ElementDefinitionBindingComponent();
