 ...
 * information, the HAPI's {@link ca.uhn.fhir.rest.api.MethodOutcome} is used.
 * </p>
 *
 * <p>
 * Implementations of this interface should prefer to throw the exceptions located in the
 * {@link ca.uhn.fhir.rest.server.exceptions} package.
 * </p>
 *
 * <p>
 * If a given operations is not supported, implementations should throw an
 * UnsupportedOperationException. The capabilities operation, if supported, should return the set of
 * supported interactions. If capabilities is not supported, the components in this repository will
 * try to invoke operations with "sensible" defaults. For example, by using the standard FHIR search
 * parameters. Discussion is on-going to determine what a "sensible" minimal level of support for
