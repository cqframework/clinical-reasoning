...
class ProfileAnswerConversionTest {
    ...
    void extensionSliceCannotBorrowRootValueConstraint(FhirVersionEnum version) {
        ...
        processor.processSliceItem(
                ...
                identifiers,
                properties);
        // An unresolved Extension profile cannot borrow the root's CodeableConcept constraint.
        // Coding is a legal base Extension value and must be preserved.
        assertEquals(
                1,
                context.newFhirPath()
        ...
        assertEquals(1, values.size(), parser.encodeResourceToString(observation));
        assertEquals("Coding", values.get(0).fhirType());
    }
        ...
        for (var profileForm :
            ...
            for (var answerPath : profileForm.equals("sliced")
                ...
                if (profileForm.equals("sliced")) {
                    assertTrue(
                            actualValue.isEmpty(),
                            "A Quantity slice must not receive a CodeableConcept from a sibling slice");
                    continue;
                }
