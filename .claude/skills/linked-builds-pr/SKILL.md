...
description: Use when the user asks to create PRs, commit and push, or submit changes. Handles cross-repo PR creation with Depends-On directives, transitive linked builds, and topological ordering.
---
...
downstream PR declaring `Depends-On:` for its upstream dependencies. Once all PRs are created,
go back and update the upstream PRs to indicate they block downstream work by appending a `Blocks:` section.
...
If no linked builds are active (i.e., `local.properties` doesn't exist, is empty, 
comments out all properties, or only has unrelated properties), then no cross-repo PRs are needed, 
exit this workflow, and create a single PR for this repo as usual.
