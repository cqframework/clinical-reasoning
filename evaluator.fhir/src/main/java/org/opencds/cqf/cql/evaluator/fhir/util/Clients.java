...
public class Clients {
    ...
    public static IGenericClient forUrl(FhirContext theFhirContext, String theUrl,
        ...
        theFhirContext.getRestfulClientFactory()
                .setServerValidationMode(theServerValidationModeEnum);
    ...
    public static IGenericClient forEndpoint(FhirContext theFhirContext,
        ...
        if (theEndpoint.hasHeader()) {
            List<String> headerList = theEndpoint.getHeader().stream()
                    .map(headerString -> headerString.asStringValue()).collect(Collectors.toList());
     ...
     */
    public static IGenericClient forEndpoint(FhirContext theFhirContext,
            org.hl7.fhir.r4.model.Endpoint theEndpoint) {
        checkNotNull(theFhirContext);
        checkNotNull(theEndpoint);
     ...
     */
    public static IGenericClient forEndpoint(FhirContext theFhirContext,
            org.hl7.fhir.r5.model.Endpoint theEndpoint) {
        checkNotNull(theFhirContext);
        checkNotNull(theEndpoint);
    ...
    public static void registerHeaders(IGenericClient theClient, List<String> theHeaderList) {
        ...
        for (Map.Entry<String, String> entry : headerMap.entrySet()) {
            IClientInterceptor headInterceptor =
                    new HeaderInjectionInterceptor(entry.getKey(), entry.getValue());
     ...
     */
    public static void registerBasicAuth(IGenericClient theClient, String theUsername,
            String thePassword) {
        checkNotNull(theClient, "theClient is required");

        if (theUsername != null) {
            BasicAuthInterceptor authInterceptor =
                    new BasicAuthInterceptor(theUsername, thePassword);
