...
- Line 506: `calculateContinuousVariableAggregateQuantity()` calls for stratifier scoring (without groupDef parameter)
...
class R4MeasureReportScorerQuantityDefTest {
    ...
    void testAggregateSum_WithNullValues() {
        List<QuantityDef> quantities = Arrays.asList(
            new QuantityDef(10.0),
            new QuantityDef(null),  // null value
            new QuantityDef(20.0),
            new QuantityDef(null)   // null value
        );

        // Test that aggregate() filters out nulls and sums remaining values
        // Expected: 10.0 + 20.0 = 30.0
        // This test will verify the filter(Objects::nonNull) logic works
    }
    ...
    void testAggregateAvg_WithNullValues() {
        List<QuantityDef> quantities = Arrays.asList(
            new QuantityDef(10.0),
            new QuantityDef(null),
            new QuantityDef(20.0),
            new QuantityDef(30.0)
        );
    ...
    void testAggregateMedian_WithNullValues_OddCount() {
        List<QuantityDef> quantities = Arrays.asList(
            ...
            new QuantityDef(30.0),
            new QuantityDef(null),
            new QuantityDef(40.0),
            new QuantityDef(50.0)
        );
    ...
    void testStratifierScoring_WithQuantityDef() {
        ...
        patient1Results.put("resource1", new QuantityDef(10.0));

        Map<String, Object> patient2Results = new HashMap<>();
        patient2Results.put("resource2", new QuantityDef(20.0));
    ...
    void testCollectQuantities_FromNestedMaps() {
        ...
        subjectResults.put("resource1", new QuantityDef(10.0));
        subjectResults.put("resource2", new QuantityDef(20.0));
    ...
    void testCollectQuantities_MixedTypes() {
        ...
        validMap.put("resource1", new QuantityDef(10.0));
    ...
    private Map<Object, QuantityDef> createMapWithQuantityDef(String key, Double value) {
        ...
        map.put(key, new QuantityDef(value));
        return map;
    }
}
```
...
2. **Removed Unused Parameters**:
   - Removed unused `groupDef` parameter from `calculateContinuousVariableAggregateQuantity()`:
   ...
   - Removed unused `groupDef` parameter from `scoreContinuousVariable()`:
     - **Before**: `scoreContinuousVariable(String measureUrl, MeasureReportGroupComponent mrgc, GroupDef groupDef, PopulationDef populationDef)`
     - **After**: `scoreContinuousVariable(String measureUrl, MeasureReportGroupComponent mrgc, PopulationDef populationDef)`
     - Updated 1 call site in the scoring switch statement
   - Updated PRP Phase 5 examples to reflect these changes

3. **Value Variable Capture and Simplified Boxing**: Captured `quantityDef.value()` in variable:
   - **Before**:
     ```java
     if (quantityDef.value() != null) {
         quantity.setValue(quantityDef.value());
     }
     ```
   - **After**:
     ```java
   ...
   - Benefits: Avoids calling `quantityDef.value()` twice, clearer intent

### Simplification (2025-11-28 - Third Round):

**Removed unit, system, and code fields from QuantityDef** to simplify the implementation:
- QuantityDef now contains only a `value` field
- Converter implementations simplified - no longer extract or populate unit/system/code
- Aggregation logic simplified - no longer preserves unit/system/code from first element
- Test cases simplified - removed all assertions on unit/system/code fields

### Files Modified (Post-Implementation):
- `QuantityDef.java` - Added toString(), simplified to value-only
- `R4ContinuousVariableObservationConverter.java` - Fixed NPE, renamed variable, captured value in variable, removed unit/system/code handling
- `Dstu3ContinuousVariableObservationConverter.java` - Fixed NPE, renamed variable, captured value in variable, removed unit/system/code handling
- `R4MeasureReportScorer.java` - Fixed NPE, added converter field, added import, updated aggregate() pattern, removed unused parameter, removed unit/system/code preservation
- `QuantityDefTest.java` - Added 4 instance equality tests, simplified to value-only tests
- `R4ContinuousVariableObservationConverterTest.java` - Simplified one test, removed unit/system/code assertions
- `Dstu3ContinuousVariableObservationConverterTest.java` - New file (14 tests), value-only assertions
